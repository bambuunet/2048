...
console.log('a')
    var playTimer = setInterval(function(){
      ...
      for(var i in weight){
        var direction = weight[i]['direction'];
      ...
      if(checkFinish()) clearInterval(playTimer);
    }, 1);
  });
...
function init(){
  ...
  wih = getCsv('./csv/status64layer2/wih.csv');
  who = getCsv('./csv/status64layer2/who.csv');
...
function getWeight(){
  var neuronResult = calcNeuron();
  var weight = [
    {direction:'up', weight: Math.round(neuronResult[0][0] * 10000) / 100},
    {direction:'right', weight: Math.round(neuronResult[1][0] * 10000) / 100},
    {direction:'down', weight: Math.round(neuronResult[2][0] * 10000) / 100},
    {direction:'left', weight: Math.round(neuronResult[3][0] * 10000) / 100}
  ];
  $$('#up').text(weight[0]['weight']);
  $$('#right').text(weight[1]['weight']);
  $$('#down').text(weight[2]['weight']);
  $$('#left').text(weight[3]['weight']);
  return weight;
}